"""Unit tests for skills/process/tools.py — all psutil calls are mocked."""

from __future__ import annotations

from unittest.mock import MagicMock, patch

from skills.process.tools import find_process_by_name, kill_process, list_processes

from tests.skills.conftest import raw

_list_processes = raw(list_processes)
_find_process_by_name = raw(find_process_by_name)
_kill_process = raw(kill_process)
...
def test_list_processes_returns_table(
    ...
    # Patch time.sleep at the module level where it is imported inside the function
    with patch("time.sleep"):
        result = _list_processes()
...
def test_find_process_by_name_match(mock_iter: MagicMock) -> None:
    ...
    result = _find_process_by_name("notepad")
    assert result["status"] == "ok"
...
def test_find_process_by_name_no_match(mock_iter: MagicMock) -> None:
    mock_iter.return_value = []
    result = _find_process_by_name("doesnotexist")
    assert result["status"] == "ok"
